...
// Used to enable or disable functions needed for rough telescope position
...
// Address
#define BNO_W_ADDR 0x94
#define BNO_R_ADDR 0x95
...
typedef enum {
  ...
  UpgradeApp, // 1 – Upgrade Application Mode; upgrade the application image in
              // flash.
  ValidateImage, // 2 – Validate Image Mode; validate an application image
                 // without updating the flash
  LauchApp, // 3 – Launch Application; launch the application image in flash
  BootInvalid
} BNO_BootMode_t;

typedef enum {
  MOTION_INTENT_UNKNOWN = 0, // FME_MOBILE_MOTION_INTENT_UNKNOWN – this is the
                             // initial state assumed by the sensor hub
...
typedef struct __attribute__((packed)) {
  uint8_t
      Severity; // Severity – the severity of the error currently being reported
  uint8_t SeqNo;  // Error sequence number. A monotonically incrementing uin8_t
                  // that counts all the errors generated for the reported
                  // severity. It may rollover.
  uint8_t Source; // Error source. 0 – reserved, 1 – MotionEngine, 2 –
                  // MotionHub, 3 – SensorHub, 4 – Chip level executable, 5-254
                  // reserved. 255 – no error to report.
...
#if defined(ROTATION_VECTOR) || defined(GEOMAGNETIC_ROTATION_VECTOR) ||        \
    defined(ARVR_STABILIZED_RV)
...
typedef struct __attribute__((packed)) {
  ...
  uint8_t sequence; // The sequence number increments once for each report sent.
                    // Gaps in the sequence numbers indicate missing or dropped
                    // reports.
  uint8_t status; // bits 7-5: reserved, 4-2: exponent delay, 1-0: Accuracy 0 -
                  // Unreliable 1 - Accuracy low 2 - Accuracy medium 3 -
                  // Accuracy high
...
HAL_StatusTypeDef BNO_setFeature(const uint8_t reportID,
...
#endif
#ifdef RAW_GYROSCOPE
// Raw uncalibrated gyroscope (ADC units). Data direct from the gyroscope, used
// for testing
...
#endif
#ifdef RAW_MAGNETOMETER
// Raw magnetic field measurement (in ADC units). Direct data from the
// magnetometer, used for testing
BNO_RawMagnetometer_t getRawMagnetometer(void);
#endif
#ifdef MAGNETIC_FIELD_CALIBRATED
// Magnetic field calibrated (in µTesla). The fully calibrated magnetic field
// measurement.
...
// accelerometer and the gyroscope (i.e. no magnetometer). The game rotation
// vector does not use the magnetometer to correct the gyroscopes drift in yaw.
// This is a deliberate omission (as specified by Google) to allow gaming
// applications to use a smoother representation of the orientation without the
// jumps that an instantaneous correction provided by a magnetic field update
// could provide.
BNO_RotationVector_t getGameRotationVector(void);
#endif
#ifdef GEOMAGNETIC_ROTATION_VECTOR
// THE GEOMAGNETIC ROTATION VECTOR is an orientation output that is expressed as
// a quaternion referenced to magnetic north and gravity. It is produced by
// fusing the outputs of the accelerometer and magnetometer. The gyroscope is
// specifically excluded in order to produce a rotation vector output using less
// power than is required to produce the rotation vector of section 2.2.4. The
// consequences of removing the gyroscope are: • Less responsive output since
// the highly dynamic outputs of the gyroscope are not used • More errors in the
// presence of varying magnetic fields
BNO_RotationVectorWAcc_t getGeoMagRotationVector(void);
#endif
#ifdef PRESSURE
// The pressures sensor reports atmospheric pressure. The units are
// hectopascals.
float getPressure(void);
#endif
#ifdef AMBIENT_LIGHT
// The ambient light sensor reports the measures the amount of light entering
// the sensor. The units are lux.
float getAmbientLight(void);
#endif
#ifdef HUMIDITY
// The humidity sensor reports relative humidity in the ambient air. The units
// are percent.
float getHumidity(void);
#endif
#ifdef PROXIMITY
// The proximity sensor reports distance from the device. The units are
// centimeters.
...
#endif
#ifdef STEP_DETECTOR
// The step detector reports steps detected. Each report indicates a single
// step.
...
#endif
#ifdef SIGNIFICANT_MOTION
// The significant motion detector sends a single report when it detects
// significant motion. It automatically turns itself off after sending its
// single report.
...
#endif
#ifdef STABILITY_DETECTOR
// The stability detector sends a report each time it detects entry in or exit
// from a stable state.
...
#endif
#ifdef SLEEP_DETECTOR
// The sleep detector sends a report each time it detects a change in sleep
// state.
...
#endif
#ifdef POCKET_DETECTOR
// The pocket detector sends a report each time it detects entry in or exit from
// a pocket state.
uint16_t getPocketDetector(void);
#endif
#ifdef CIRCLE_DETECTOR
// The circle detector sends a report each time it detects a double circle
// gesture.
uint16_t getCircleDetector(void);
#endif
#ifdef HEART_RATE_MONITOR
// The heart rate monitor measures the user’s heart rate reported in beats per
// minute (bpm)
...
// (typically augmented or virtual reality applications) where a sudden jump can
// be disturbing, the output is adjusted to prevent these jumps in a manner that
// takes account of the velocity of the sensor system. This process is called
// AR/VR stabilization. An FRS (Flash Record System – see Figure 1-31) record is
// provided to allow configuration of this feature
...
// sudden jump can be disturbing, the output is adjusted to prevent these jumps
// in a manner that takes account of the velocity of the sensor system. This
// process is called AR/VR stabilization. An FRS (Flash Record System – see
// Figure 1-31) record is provided to allow configuration of this feature.
BNO_RotationVector_t BNO_getArVrStabilizedGRV(void);
#endif
#ifdef GYRO_INTEGRATED_RV
// GYRO ROTATION VECTOR
// Head tracking systems within a virtual reality headset require low latency
// processing of motion. To facilitate this, the BNO08X can provide a rotation
// vector at rates up to 1kHz. The gyro rotation Vector provides an alternate
// orientation to the standard rotation vector. Compared to the standard
// rotation vector the gyro rotation vector has an optimized processing path and
// correction style (correction is the adjustments made to the output based on
// more accurate estimates of gravity, mag field, angular velocity) that is
// suitable for head tracking applications.
...
#endif
#ifdef DEAD_RECKONING_POSE
// The Dead Reckoning Pose report provides the linear position, angular
// position, linear velocity, and angular velocity as determined by the Dead
// Reckoning system.
