...
// Get microseconds
static uint32_t timeNowUs(void) { return __HAL_TIM_GET_COUNTER(&htim2); }
...
static void quaternionUpdate(const BNO_RotationVectorWAcc_t *inRotVector) {
  ...
  float norm = invSqrt(
      inRotVector->I * inRotVector->I + inRotVector->J * inRotVector->J +
      inRotVector->K * inRotVector->K + inRotVector->Real * inRotVector->Real);
...
// Send packet to IMU
  ...
  if (HAL_I2C_Master_Transmit_DMA(&hi2c1, BNO_W_ADDR, bufferIO, dataLength) !=
      HAL_OK) {
...
#else
  if (HAL_I2C_Master_Transmit(&hi2c1, BNO_W_ADDR, bufferIO, dataLength,
                              I2C_TIMEOUT) != HAL_OK) {
...
// First, receive the header (4 bytes) to determine the full packet size
#ifdef USE_I2C_DMA
                                       // i2c1_transfer_complete = 0;  // Reset
                                       // DMA transfer complete flag
  if (HAL_I2C_Master_Receive_DMA(&hi2c1, BNO_R_ADDR, bufferIO, HEADER_SIZE) !=
      HAL_OK) {
...
#else
  if (HAL_I2C_Master_Receive(&hi2c1, BNO_R_ADDR, bufferIO, HEADER_SIZE,
                             I2C_TIMEOUT) != HAL_OK) {
  ...
  delay_us(RESET_DELAY); // Delay 100 microseconds before receiving the rest of
                         // the packet

// Now, receive the full packet based on the calculated size
#ifdef USE_I2C_DMA
                         // i2c1_transfer_complete = 0;  // Reset DMA transfer
                         // complete flag
  if (HAL_I2C_Master_Receive_DMA(&hi2c1, BNO_R_ADDR, bufferIO,
                                 rxPacketLength) != HAL_OK) {
    return HAL_ERROR; // Return error if DMA reception fails
  }
  // Wait for DMA transfer to complete
  // while (!i2c1_transfer_complete);
...
#else
  if (HAL_I2C_Master_Receive(&hi2c1, BNO_R_ADDR, bufferIO, rxPacketLength,
                             I2C_TIMEOUT) != HAL_OK) {
    return HAL_ERROR; // Return error if reception fails
  }
#endif
...
// bufferIO - 10 Sequence number increments once for each report sent. Gaps in
// the sequence numbers indicate missing or dropped reports. bufferIO - 11
// Status bits 7-5: reserved, 4-2: exponent delay, 1-0: Accuracy
static void getSensorValue(void) {
...
#ifdef ACCELEROMETER
  case ACCELEROMETER:
    sensorData.SenVal.Accelerometer.X =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q8;
    sensorData.SenVal.Accelerometer.Y =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q8;
    sensorData.SenVal.Accelerometer.Z =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q8;
    break;
#endif
#ifdef LINEAR_ACCELERATION
  case LINEAR_ACCELERATION:
    sensorData.SenVal.LinearAcceleration.X =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q8;
    sensorData.SenVal.LinearAcceleration.Y =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q8;
    sensorData.SenVal.LinearAcceleration.Z =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q8;
    break;
#endif
#ifdef GRAVITY
  ...
  case GYROSCOPE_CALIBRATED:
    sensorData.SenVal.Gyroscope.X =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q9;
    sensorData.SenVal.Gyroscope.Y =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q9;
    sensorData.SenVal.Gyroscope.Z =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q9;
    break;
#endif
#ifdef GYROSCOPE_UNCALIBRATED
  case GYROSCOPE_UNCALIBRATED:
    sensorData.SenVal.GyroscopeUncal.X =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q9;
    sensorData.SenVal.GyroscopeUncal.Y =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q9;
    sensorData.SenVal.GyroscopeUncal.Z =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q9;
    sensorData.SenVal.GyroscopeUncal.BiasX =
        (float)(*(int16_t *)&bufferIO[19]) * SCALE_Q9;
    sensorData.SenVal.GyroscopeUncal.BiasY =
        (float)(*(int16_t *)&bufferIO[21]) * SCALE_Q9;
    sensorData.SenVal.GyroscopeUncal.BiasZ =
        (float)(*(int16_t *)&bufferIO[23]) * SCALE_Q9;
    break;
#endif
  ...
  case MAGNETIC_FIELD_CALIBRATED:
    sensorData.SenVal.MagneticField.X =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q4;
    sensorData.SenVal.MagneticField.Y =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q4;
    sensorData.SenVal.MagneticField.Z =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q4;
    break;
#endif
#ifdef MAGNETIC_FIELD_UNCALIBRATED
  case MAGNETIC_FIELD_UNCALIBRATED:
    sensorData.SenVal.MagneticFieldUncal.X =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q4;
    sensorData.SenVal.MagneticFieldUncal.Y =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q4;
    sensorData.SenVal.MagneticFieldUncal.Z =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q4;
    sensorData.SenVal.MagneticFieldUncal.BiasX =
        (float)(*(int16_t *)&bufferIO[19]) * SCALE_Q4;
    sensorData.SenVal.MagneticFieldUncal.BiasY =
        (float)(*(int16_t *)&bufferIO[21]) * SCALE_Q4;
    sensorData.SenVal.MagneticFieldUncal.BiasZ =
        (float)(*(int16_t *)&bufferIO[23]) * SCALE_Q4;
    break;
#endif
#ifdef ROTATION_VECTOR
  case ROTATION_VECTOR:
#ifdef ENABLED_REPORTS
    sensorData.SenVal.RotationVector.I =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q14;
    sensorData.SenVal.RotationVector.J =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q14;
    sensorData.SenVal.RotationVector.K =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q14;
    sensorData.SenVal.RotationVector.Real =
        (float)(*(int16_t *)&bufferIO[19]) * SCALE_Q14;
    sensorData.SenVal.RotationVector.Accuracy =
        (float)(*(int16_t *)&bufferIO[21]) * SCALE_Q14;
    // Update Euler
  ...
  case GAME_ROTATION_VECTOR:
    sensorData.SenVal.GameRotationVector.I =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q14;
    sensorData.SenVal.GameRotationVector.J =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q14;
    sensorData.SenVal.GameRotationVector.K =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q14;
    sensorData.SenVal.GameRotationVector.Real =
        (float)(*(int16_t *)&bufferIO[19]) * SCALE_Q14;
    break;
#endif
#ifdef GEOMAGNETIC_ROTATION_VECTOR
  case GEOMAGNETIC_ROTATION_VECTOR:
    sensorData.SenVal.GeoMagRotationVector.I =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q14;
    sensorData.SenVal.GeoMagRotationVector.J =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q14;
    sensorData.SenVal.GeoMagRotationVector.K =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q14;
    sensorData.SenVal.GeoMagRotationVector.Real =
        (float)(*(int16_t *)&bufferIO[19]) * SCALE_Q14;
    sensorData.SenVal.GeoMagRotationVector.Accuracy =
        (float)(*(int16_t *)&bufferIO[21]) * SCALE_Q14;
    break;
#endif
#ifdef PRESSURE
...
#ifdef AMBIENT_LIGHT
  case AMBIENT_LIGHT:
    sensorData.SenVal.AmbientLight =
        (float)(*(int32_t *)&bufferIO[13]) * SCALE_Q8;
    break;
#endif
#ifdef HUMIDITY
  case HUMIDITY:
...
#ifdef TEMPERATURE
  case TEMPERATURE:
    sensorData.SenVal.Temperature =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q7;
    break;
#endif
#ifdef RESERVED
  case RESERVED:
  ...
  case PERSONAL_ACTIVITY_CLASSIFIER:
    ...
    sensorData.SenVal.PersonalActivityClassifier.LastPage =
        ((bufferIO[13] & 0x80) != 0);
    sensorData.SenVal.PersonalActivityClassifier.MostLikelyState = bufferIO[14];
    // ToDo remove for loop, use pointer
    for (int n = 0; n < 10; n++) {
      sensorData.SenVal.PersonalActivityClassifier.Confidence[n] =
          bufferIO[15 + n];
    }
    break;
#endif
  ...
  case ARVR_STABILIZED_RV:
    sensorData.SenVal.ArVrStabilizedRV.I =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q14;
    sensorData.SenVal.ArVrStabilizedRV.J =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q14;
    sensorData.SenVal.ArVrStabilizedRV.K =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q14;
    sensorData.SenVal.ArVrStabilizedRV.Real =
        (float)(*(int16_t *)&bufferIO[19]) * SCALE_Q14;
    sensorData.SenVal.ArVrStabilizedRV.Accuracy =
        (float)(*(int16_t *)&bufferIO[21]) * SCALE_Q12;
    break;
#endif
#ifdef ARVR_STABILIZED_GRV
  case ARVR_STABILIZED_GRV:
    sensorData.SenVal.ArVrStabilizedGRV.I =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q14;
    sensorData.SenVal.ArVrStabilizedGRV.J =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q14;
    sensorData.SenVal.ArVrStabilizedGRV.K =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q14;
    sensorData.SenVal.ArVrStabilizedGRV.Real =
        (float)(*(int16_t *)&bufferIO[19]) * SCALE_Q14;
    break;
#endif
#ifdef GYRO_INTEGRATED_RV
  case GYRO_INTEGRATED_RV:
    sensorData.SenVal.GyroIntegratedRV.I =
        (float)(*(int16_t *)&bufferIO[9]) * SCALE_Q14;
    sensorData.SenVal.GyroIntegratedRV.J =
        (float)(*(int16_t *)&bufferIO[11]) * SCALE_Q14;
    sensorData.SenVal.GyroIntegratedRV.J =
        (float)(*(int16_t *)&bufferIO[13]) * SCALE_Q14;
    sensorData.SenVal.GyroIntegratedRV.Real =
        (float)(*(int16_t *)&bufferIO[15]) * SCALE_Q14;
    sensorData.SenVal.GyroIntegratedRV.AngleVelX =
        (float)(*(int16_t *)&bufferIO[17]) * SCALE_Q10;
    sensorData.SenVal.GyroIntegratedRV.AngleVelY =
        (float)(*(int16_t *)&bufferIO[19]) * SCALE_Q10;
    sensorData.SenVal.GyroIntegratedRV.AngleVelZ =
        (float)(*(int16_t *)&bufferIO[21]) * SCALE_Q10;
    break;
#endif
#ifdef IZRO_MOTION_REQUEST
  case IZRO_MOTION_REQUEST:
    sensorData.SenVal.IzroRequest.Intent = (BNO_IZroMotionIntent_t)bufferIO[13];
    sensorData.SenVal.IzroRequest.Request =
        (BNO_IZroMotionRequest_t)bufferIO[14];
    break;
#endif
  ...
  case DEAD_RECKONING_POSE:
    ...
    sensorData.SenVal.DeadReckoningPose.LinPosX =
        (float)(*(int32_t *)&bufferIO[17]) * SCALE_Q17;
    sensorData.SenVal.DeadReckoningPose.LinPosY =
        (float)(*(int32_t *)&bufferIO[21]) * SCALE_Q17;
    sensorData.SenVal.DeadReckoningPose.LinPosZ =
        (float)(*(int32_t *)&bufferIO[25]) * SCALE_Q17;

    sensorData.SenVal.DeadReckoningPose.I =
        (float)(*(int32_t *)&bufferIO[29]) * SCALE_Q30;
    sensorData.SenVal.DeadReckoningPose.J =
        (float)(*(int32_t *)&bufferIO[33]) * SCALE_Q30;
    sensorData.SenVal.DeadReckoningPose.K =
        (float)(*(int32_t *)&bufferIO[37]) * SCALE_Q30;
    sensorData.SenVal.DeadReckoningPose.Real =
        (float)(*(int32_t *)&bufferIO[41]) * SCALE_Q30;

    sensorData.SenVal.DeadReckoningPose.LinVelX =
        (float)(*(int32_t *)&bufferIO[45]) * SCALE_Q25;
    sensorData.SenVal.DeadReckoningPose.LinVelY =
        (float)(*(int32_t *)&bufferIO[49]) * SCALE_Q25;
    sensorData.SenVal.DeadReckoningPose.LinVelZ =
        (float)(*(int32_t *)&bufferIO[53]) * SCALE_Q25;

    sensorData.SenVal.DeadReckoningPose.AngleVelX =
        (float)(*(int32_t *)&bufferIO[57]) * SCALE_Q25;
    sensorData.SenVal.DeadReckoningPose.AngleVelY =
        (float)(*(int32_t *)&bufferIO[61]) * SCALE_Q25;
    sensorData.SenVal.DeadReckoningPose.AngleVelZ =
        (float)(*(int32_t *)&bufferIO[65]) * SCALE_Q25;
    break;
#endif
#ifdef WHEEL_ENCODER
  case WHEEL_ENCODER:
...
static HAL_StatusTypeDef processResponse(void) {
  ...
  case REPORT_GET_FEATURE_RESPONSE: // 0xFC
    sensorFeatures = *(BNO_Feature_t *)&bufferIO[5];
    return HAL_OK;
    break;
  ...
  if (sendPacket(sendChannel) == HAL_OK) {
    ...
    while (retry) {
      if (waitForPacket() == HAL_OK) {
        if ((bufferIO[2] == receiveChannel) &&
            (bufferIO[4] == expectedResponse)) {
...
// During reset or power-on sequence, the bootloader first checks the status of
// the BOOTN pin. If the pin is pulled low during reset or poweron, the BNO08X
// will enter the bootloader mode. If the BOOTN pin is pulled high, then the
// bootloader starts the application
HAL_StatusTypeDef BNO_Init(void) {
  ...
  BNO_RST_On;
  HAL_Delay(RESET_DELAY);
  // If we got the initiate packet we make a soft reset
  if (waitForPacket() == HAL_OK) {
    if (processResponse() == HAL_OK) {
      // Wait for intterupt
      if (waitInt()) {
        if (BNO_Reset() == HAL_OK) {
          // Finally, we want to interrogate the device about its model and
          // version.
          BNO_On();
          return getID();
        }
...
HAL_StatusTypeDef BNO_setHighAccuracyMode(void) {
  const uint32_t reportIntervalUs =
      200000; // 200 ms = 200,000 microseconds (5Hz)

  // Enable high accuracy for the accelerometer, gyroscope, and magnetometer
  if (BNO_configureCalibration(CALIBRATE_ACCEL_GYRO_MAG) == HAL_OK) {
    // Set high accuracy mode for the Magnetometer (MAGNETIC_FIELD_CALIBRATED)
    if (BNO_setFeature(MAGNETIC_FIELD_CALIBRATED, reportIntervalUs, 0) ==
        HAL_OK) {
      ...
      if (BNO_setFeature(ROTATION_VECTOR, reportIntervalUs, 0) == HAL_OK) {
        ...
        if (BNO_setFeature(ACCELEROMETER, reportIntervalUs, 0) != HAL_OK)
          return HAL_ERROR;
        if (BNO_setFeature(GYROSCOPE_CALIBRATED, reportIntervalUs, 0) != HAL_OK)
          return HAL_ERROR;
...
uint8_t BNO_getSensorEventID(void) { return sensorData.sensorId; }

// Soft reset the sensor
HAL_StatusTypeDef BNO_Reset(void) {
  if (sendExecutable(COMMAND_INITIALIZE_RESET) !=
      HAL_OK) { // Write 1 byte to chan EXE
    return HAL_ERROR;
  }
...
// Turn sensor ON
HAL_StatusTypeDef BNO_On(void) { return sendExecutable(COMMAND_INITIALIZE_ON); }

// When sleep command is issued all sensors that are configured as always on or
// wake (see 1.3.5.1) will continue to operate all, other sensors will be
// disabled
...
BNO_productID_t BNO_getProductID(void) { return myID; }

// Length in 32-bit words of the record to be written. If the length is set to 0
// then the record is erased
...
HAL_StatusTypeDef BNO_getErrors(void) {
  ...
  // bufferIO[7] = 0; // he severity of errors to report. Errors of this
  // severity and higher will be reported. 0 – highest priority
  if (waitForCommandResponse() == HAL_OK) {
    // Error source. 0 – reserved, 1 – MotionEngine, 2 – MotionHub, 3 –
    // SensorHub, 4 – Chip level executable, 5-254 reserved. 255 – no error to
    // report.
...
HAL_StatusTypeDef BNO_TareNow(const BNO_TareAxis_t axis,
                              const BNO_TareRV_t vector) {
...
// This command instructs SH-2 to persist the results of the last tare operation
// to flash for use at the next system restart.
...
// The rotation vector is a signed, 16-bit 2’s-complement fixed point number
// with a Q-point of 14.
HAL_StatusTypeDef BNO_TareSetReorientation(const double X, const double Y,
...
// The sensor hub responds to the Initialize command with an Initialize
// Response. In the case where the sensor hub reinitializes itself, this
// response is unsolicited. An unsolicited response is also generated after
// startup
HAL_StatusTypeDef BNO_Initialize(void) {
  ...
  bufferIO[7] =
      COMMAND_INITIALIZE_RESET; // 1 Reinitialize the entire sensor hub.
  return waitForCommandResponse();
}
...
// This command is sent by the host to configure the ME calibration of the
// accelerometer, gyro and magnetometer giving the host the ability to control
// when calibration is performed.
HAL_StatusTypeDef BNO_configureCalibration(const uint8_t sensors) {
  resetHeader(REPORT_COMMAND_REQUEST);
  bufferIO[5] = cmdSeqNo++;
...
// This command is sent by the host to request the enable/disable state of the
...
// The Configure Periodic DCD Save command configures the automatic saving of
// DCD. There is no response to this command. This command does not inhibit the
// Save DCD command. 0 Enable, 1 Disable
...
// The Get Oscillator Type command is used to get information about the
// oscillator type used in the clock system of the SH-2
BNO_Oscillator_t BNO_getOscillatorType(void) {
  resetHeader(REPORT_COMMAND_REQUEST);
  bufferIO[5] = cmdSeqNo++;
  bufferIO[6] = COMMAND_OSCILLATOR;
  if (waitForCommandResponse() == HAL_OK) {
    if (oscillatorType > ExternalClock)
      return OscillatorError;
  }
...
HAL_StatusTypeDef BNO_simpleCalibration(const uint32_t usInterval,
                                        const uint16_t calibrationTimeMs) {
  if (BNO_configureCalibration(CALIBRATE_ON_TABLE) == HAL_OK) {
    resetHeader(REPORT_COMMAND_REQUEST);
    bufferIO[5] = cmdSeqNo++;
    ...
    *(uint32_t *)&bufferIO[8] = usInterval;
    if (waitForCommandResponse() ==
        HAL_OK) { // CHANNEL_CONTROL, REPORT_COMMAND_RESPONSE
      if (!calibrationStatus.Status) {
        uint32_t endCalibrationTime = HAL_GetTick() + calibrationTimeMs;
        while (HAL_GetTick() < endCalibrationTime) {
        } // Just wait
...
// ToDo Check functionality Not getting a reponce to start
// The bootloader operating mode request is used to request various operating
// modes of the FSP200 bootloader
HAL_StatusTypeDef BNO_setBootMode(const BNO_BootMode_t mode) {
  resetHeader(REPORT_COMMAND_REQUEST);
  bufferIO[5] = cmdSeqNo++;
  bufferIO[6] = COMMAND_BOOTLOADER;          // 0x0D – Bootloader command
  bufferIO[7] = COMMAND_BOOTLOADER_MODE_REQ; // 0x00 – Sub-command: Bootloader
...
BNO_BootMode_t BNO_getBootMode(void) {
  ...
  bufferIO[5] = cmdSeqNo++;
  bufferIO[6] = COMMAND_BOOTLOADER;            // 0x0D – Bootloader command
  bufferIO[7] = COMMAND_BOOTLOADER_STATUS_REQ; // 0x01 – Sub-command: Bootloader
                                               // Status Request
  if (waitForCommandResponse() == HAL_OK) {
...
// The interactive calibration feature requires that the sensor hub be told of
// the device’s intended motion.
HAL_StatusTypeDef BNO_interactiveCalibration(const BNO_MotionIntent_t intent) {
  resetHeader(REPORT_COMMAND_REQUEST);
  bufferIO[5] = cmdSeqNo++;
  bufferIO[6] =
      COMMAND_INTERACTIVE_CAL_REQ; // 0x0E – Interactive Calibration command
  bufferIO[7] = intent;            // Motion intent
  return sendPacket(CHANNEL_CONTROL);
}

#ifdef WHEEL_ENCODER
// Provide a single sample of wheel encoder data. No response is sent for this
// command.
HAL_StatusTypeDef BNO_WheelRequest(const uint8_t wheelIndex,
                                   const uint32_t timeStampUs,
                                   const int16_t wheelData,
                                   const uint8_t dataType) {
  resetHeader(REPORT_COMMAND_REQUEST);
  bufferIO[5] = cmdSeqNo++;
  bufferIO[6] = COMMAND_WHEEL_REQ; // 0x0F – Wheel Request
  bufferIO[7] = wheelIndex; // Wheel Index (0 = Left Wheel, 1 = Right Wheel)
  *(uint32_t *)&bufferIO[8] = timeStampUs; // The timestamp is a 32-bit unsigned
                                           // timestamp in microseconds.
  *(int16_t *)&bufferIO[12] = wheelData;   // The timestamp is a 32-bit unsigned
                                           // timestamp in microseconds.
...
// This is sent from the host to the hub to trigger a flush of outstanding data
// from a given sensor
    ...
    if (BNO_setFeature(MAGNETIC_FIELD_CALIBRATED, 100000, 0) == HAL_OK) {
      if (BNO_setFeature(GAME_ROTATION_VECTOR, 100000, 0) == HAL_OK) {
        ...
        while (HAL_GetTick() < startTime) {
          ...
          // If both magnetometer and rotation vector have maximum accuracy,
          // stop calibration
          if ((magA == 3) && (grvA == 3)) {
            break;
          }
...
BNO_Feature_t BNO_getFeature(const uint8_t sensorID) {
  ...
  BNO_Feature_t ret = {0};
  bufferIO[5] = sensorID; // Feature Report ID. 0x01 = Accelerometer, 0x05 =
                          // Rotation vector
  if (waitForCommandResponse() == HAL_OK) {
    ret = sensorFeatures;
  }
...
// Enable features an set report time in mili seconds
HAL_StatusTypeDef BNO_setFeature(const uint8_t sensorID,
                                 const uint32_t microsBetweenReports,
                                 const uint32_t specificConfig) {
  resetHeader(
      REPORT_SET_FEATURE_COMMAND); // Set feature command. Reference page 55
                                   //	sensorFeatures.sensorID = sensorID;
                                   //	sensorFeatures.flags = 0;
                                   //	sensorFeatures.changeSensitivity = 0;
  //	sensorFeatures.reportInterval_uS = microsBetweenReports;
  //	sensorFeatures.batchInterval_uS = 0;
  //	sensorFeatures.sensorSpecific = specificConfig;
  //	*(BNO_Feature_t *)&bufferIO[5] = sensorFeatures;
  bufferIO[5] = sensorID; // Feature Report ID. 0x01 = Accelerometer, 0x05 =
                          // Rotation vector
  // bufferIO[6] = 0; // Feature flags
  //*(uint16_t *)&bufferIO[7] = 0; // Change sensitivity [absolute | relative]
  *(uint32_t *)&bufferIO[9] =
      microsBetweenReports; // Report interval (LSB) in microseconds
...
//
BNO_RollPitchYaw_t BNO_getRollPitchYaw(void) { return rpy; }
...
#endif
#ifdef GRAVITY
BNO_Accelerometer_t getGravity(void) { return sensorData.SenVal.Gravity; }
#endif
#ifdef RAW_GYROSCOPE
...
#endif
#ifdef GYROSCOPE_CALIBRATED
BNO_Gyroscope_t getGyroscope(void) { return sensorData.SenVal.Gyroscope; }
#endif
#ifdef GYROSCOPE_UNCALIBRATED
...
#endif
#ifdef TAP_DETECTOR
BNO_Tap_t getTapDetectorFlag(void) { return sensorData.SenVal.TapDetectorFlag; }
#endif
#ifdef STEP_DETECTOR
...
#endif
#ifdef STEP_COUNTER
BNO_StepCounter_t getStepCounter(void) { return sensorData.SenVal.StepCounter; }
#endif
#ifdef SIGNIFICANT_MOTION
...
#endif
#ifdef SHAKE_DETECTOR
BNO_Shake_t getShakeDetector(void) { return sensorData.SenVal.ShakeDetector; }
#endif
#ifdef FLIP_DETECTOR
uint16_t getFlipDetector(void) { return sensorData.SenVal.FlipDetector; }
#endif
#ifdef PICKUP_DETECTOR
...
#endif
#ifdef SLEEP_DETECTOR
uint8_t getSleepDetector(void) { return sensorData.SenVal.SleepDetector; }
#endif
#ifdef TILT_DETECTOR
uint16_t getTiltDetector(void) { return sensorData.SenVal.TiltDetector; }
#endif
#ifdef POCKET_DETECTOR
uint16_t getPocketDetector(void) { return sensorData.SenVal.PocketDetector; }
#endif
#ifdef CIRCLE_DETECTOR
uint16_t getCircleDetector(void) { return sensorData.SenVal.CircleDetector; }
#endif
#ifdef HEART_RATE_MONITOR
...
#endif
#ifdef IZRO_MOTION_REQUEST
BNO_IZroRequest_t getIzroRequest(void) { return sensorData.SenVal.IzroRequest; }
#endif
#ifdef RAW_OPTICAL_FLOW
BNO_RawOptFlow_t getRawOptFlow(void) { return sensorData.SenVal.RawOptFlow; }
#endif
#ifdef DEAD_RECKONING_POSE
