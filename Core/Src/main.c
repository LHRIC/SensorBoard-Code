...
bool ADC_CAN_Package(uint16_t ADC_Pin) {
  ...
  MuxBuffer[0] &= 0xF0;
  MuxBuffer[0] |= (mux_val & 0x0F);

  if (mux_val == 0) {
    // Pack channels 0-5 in mux 0 frame
    for (int i = 0; i < 5; i++) {
      uint16_t value = last_adc_values[i];
      uint8_t byte_idx =
          (i * ADC_BIT_RESOLUTION) / 8 + 1; // +1 to skip mux nibble byte
      uint8_t bit_pos = (i * ADC_BIT_RESOLUTION) % 8;

      // Pack 12-bit value across bytes
      MuxBuffer[byte_idx] &= ~(0xFF << bit_pos);
      MuxBuffer[byte_idx] |= (value & 0xFF) << bit_pos;

      if (bit_pos > 4) { // Need to use next byte
        MuxBuffer[byte_idx + 1] &= ~(0xFF >> (8 - bit_pos));
        MuxBuffer[byte_idx + 1] |= (value >> (8 - bit_pos));
      }
    }

    mux0_pending = true;
  } else {
    // Pack channels 6-7 in mux 1 frame
    for (int i = 0; i < 2; i++) {
      uint16_t value = last_adc_values[i + 6];
      uint8_t byte_idx = (i * 12) / 8 + 1; // +1 to skip mux nibble byte
      uint8_t bit_pos = (i * 12) % 8;
...
bool SendPendingCANMessages(void) {
  ...
  if (mux1_pending) {
    // Only send mux 1 if mux 0 was sent successfully
    if (!mux0_pending) {
...
int main(void) {
  ...
  while (1) {
    ...
    // Try to send pending CAN messages
    if (mux0_pending) {
      // Attempt to send mux 0 frame
      memcpy(TxData, AdcMux0Buffer, 8);
      if (HAL_CAN_AddTxMessage(&hcan, &TxHeader, TxData, &TxMailbox) ==
          HAL_OK) {
        mux0_pending = false;
    ...
    // Only send mux 1 after mux 0 is sent to maintain order
    if (!mux0_pending && mux1_pending) {
      memcpy(TxData, AdcMux1Buffer, 8);
      if (HAL_CAN_AddTxMessage(&hcan, &TxHeader, TxData, &TxMailbox) ==
          HAL_OK) {
        mux1_pending = false;
